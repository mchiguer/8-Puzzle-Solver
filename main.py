...
def getMisplacedTiles(state):
    ...
    for i in range(1,9):
        ...
        itemX = int(idx / 3)
        itemY = idx % 3
        if ((goalX != itemX) or (goalY != itemY)) : tot += 1
        else : tot += 0        
    return tot

# heuristic fonction "Rows Columns"

def getRowsColumns(state):
    tot = 0 
    for i in range(1,9):
        goal = end_state.index(str(i))
        goalX = int(goal/ 3)
...
# heuristic function "Sequences"

def getSequences(state):
    tot = 0 
    #visit the tiles in this order :
    ...
    for i in range(0,9):
        ...
        itemY = idx % 3
        idxs[itemX][itemY] = i
    if idxs[0][0] == 0 : tot += 2   # check where blank tile is
    elif idxs[0][1] == 0 : tot += 2
    elif idxs[0][1] != idxs[0][0] % 8 + 1 : tot+=2
    if idxs[0][2] == 0 : tot += 2
    elif idxs[0][2] != idxs[0][1] % 8 + 1 : tot+=2
    if idxs[1][2] == 0 : tot += 2
    elif idxs[1][2] != idxs[0][2] % 8 + 1 : tot+=2
    if idxs[1][2] == 0 : tot += 2
    elif idxs[2][2] != idxs[1][2] % 8 + 1 : tot+=2
    if idxs[2][2] == 0 : tot += 2
    elif idxs[2][1] != idxs[2][2] % 8 + 1 : tot+=2
    if idxs[2][1] == 0 : tot += 2
    elif idxs[2][0] != idxs[2][1] % 8 + 1 : tot+=2
    if idxs[2][0] == 0 : tot += 2
    elif idxs[1][0] != idxs[2][0] % 8 + 1 : tot+=2
    if idxs[1][1] != 0 : tot += 1   # check center
    if idxs[0][0] != idxs[1][0] % 8 + 1 : tot+=2

    return 3*tot

def Sequences2(state): 
    tot = 0 
    #visit the tiles in this order :
    # (0,0) (0,1) (0,2) (1,2) (2,2) (2,1) (2,0) (1,0) (1,1)
    idxs = [[0,0,0],
    ...
    if idxs[0][0]!= 0 and idxs[0][1] != idxs[0][0] % 8 + 1 : tot+=1
    if idxs[0][1]!= 0 and idxs[0][2] != idxs[0][1] % 8 + 1 : tot+=1
    if idxs[0][2]!= 0 and idxs[1][2] != idxs[0][2] % 8 + 1 : tot+=1
    if idxs[1][2]!= 0 and idxs[2][2] != idxs[1][2] % 8 + 1 : tot+=1
    if idxs[2][2]!= 0 and idxs[2][1] != idxs[2][2] % 8 + 1 : tot+=1
    if idxs[2][1]!= 0 and idxs[2][0] != idxs[2][1] % 8 + 1 : tot+=1
    if idxs[2][0]!= 0 and idxs[1][0] != idxs[2][0] % 8 + 1 : tot+=1
    if idxs[1][0]!= 0 and idxs[0][0] != idxs[1][0] % 8 + 1 : tot+=1
    else : tot+=0 
    return tot

def MisSeq(state): 
    return Sequences2(state)+getMisplacedTiles(state)
